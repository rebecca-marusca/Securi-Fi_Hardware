...
#   [0]  RSSI (int, dBm)
#   [1]  noise floor (int)
#   [2]  source MAC (bytes, 6 bytes)
#   [3]  timestamp_1 (int)
#   [4]  timestamp_2 (int)
#   [5]  CSI data (bytearray, 128 bytes = 32 subcarriers * 4 bytes each)
#   [6..23] channel/antenna metadata (ints)

_CSI_DATA_INDEX = 5
_MAC_INDEX = 2
_SUBCARRIER_COUNT = 32       # 128 bytes / 4 bytes per I+Q pair
_BYTES_PER_SAMPLE = 4        # int16 I + int16 Q


class CSICapture:
    """
    Polls wlan.csi_read() for CSI data and feeds MVSDetector.

    Runs on Core 1 via _thread.start_new_thread(capture.run, ()).
    SecuriFiNode.start() calls capture.start() first (enables CSI on
    the wlan interface), then launches run() in a thread.
    """
    ...
    def start(self) -> None:
        ...
        Does NOT start the poll loop — SecuriFiNode launches
        run() in a thread after calling this.
        """
        if self._active:
            return
        try:
    ...
    def _parse_amplitudes(self, data):
        ...
        128 bytes = 32 subcarriers * 4 bytes (int16 I + int16 Q).
