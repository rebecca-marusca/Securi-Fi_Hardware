...
def boot() -> None:
    from config import (WIFI_SSID, WIFI_PASSWORD, MASTER_MAC, TRAFFIC_RATE_PPS, ESPNOW_TX_INTERVAL_MS)
    from slave_node import SlaveNode

    node_id = read_node_id()
    print(f"[boot]: Starting as {node_id}")

    node = SlaveNode(node_id=node_id, wifi_ssid=WIFI_SSID, wifi_password=WIFI_PASSWORD, master_mac=MASTER_MAC, mq2_pin=2, mq2_threshold=1500, traffic_rate_pps=TRAFFIC_RATE_PPS)
